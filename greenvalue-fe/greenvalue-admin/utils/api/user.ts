...
import { IInvitationResponse, IInvitation, IEditUser, IUser, FetchUserParams } from '@/types/user';
import { axiosWithoutToken, axiosWithToken } from '../axios';
import { LoginResponse } from '@/types/auth';
import { PaginatedResponse } from '@/types/pagination';

export const login = async (email: string, password: string) => {
  const response = await axiosWithoutToken.post<LoginResponse>('/auth/login', {
    email,
    password,
  });
...
export const addUserInvitationApi = async (
  data: IInvitation
): Promise<IApiResponse<IInvitationResponse>> => {
  const response = await axiosWithToken.post<IApiResponse<IInvitationResponse>>(
    '/invitation',
    data
  );
  return response.data;
